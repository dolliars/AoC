...
for(let i = 0; i < entriesArr.length; i++) {
  let diff = sumToMatch - entriesArr[i];
  map.set(entriesArr[i], diff);
}

for( let j = 0; j < entriesArr.length; j++ ) {
  let diff = map.get(entriesArr[j]);
  if (map.get(diff)) {
    console.log('diff: ', diff, 'entriesArr: ', entriesArr[j] );
    console.log('2 entries multiplied = ', diff * entriesArr[j]);
  }
}
