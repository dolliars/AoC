...
function findEntriesMatch(sumToMatch) {
  for (let i = 0; i < entriesArr.length; i++) {
    for (let j = i+1; j < entriesArr.length; j++) {
      for (let k = j+1; k < entriesArr.length; k++) {
        let sum = entriesArr[i] + entriesArr[j] + entriesArr[k];
        if (sum === sumToMatch) {
          console.log(multiplier(entriesArr[i], entriesArr[j], entriesArr[k]));
        }
      }
    }
  }
