...
function splitStrings(string) {
  ...
  getSeatId(seat[0], seat[1]);
}

function findCol(colString) {
  return tree(colString, 8);
}

function findRow(rowString) {
  return tree(rowString, 128);
}
...
function scan(data) {
  data.forEach(string => splitStrings(string));
}
