...
function tinyTest() {
  ...
  console.log(add(8,2222));     // 2230
  console.log(multiply(5,5));   //   25
  console.log(multiply(10,11)); //  110
}

function checker(sum, target, x, y, z) {
  if(sum === target) {
    console.log('sum: ', sum, 'target: ', target, 'x: ', x, 'y:', y, 'z', z);
    multiply(x, y, z);
    console.log('this checks out: ', x, y, z);
  }
}

function report(array) {
  var arrayLength = array.length; // eg 200
  for (var i = 0; i <= array.length; i++) {
    for (let j = 0; j <= arrayLength ; j++) {
      for (let k = 0; k <= arrayLength; k++){
        add(array[i], array[j], array[k]);
      }
    }
  }
}
